...
const filter = `*[${productFilter}${colorFilter}${categoryFilter}${sizeFilter}${searchFilter}]`
  // await seedSanityData() // Seed data ADD ASYNC TO FUNCTION
 const prod =   await client.fetch<SanityProduct[]>(groq`${filter} ${order}
{
id,
_createdAt,
name,
        ...
        <main className="mx-auto max-w-6xl px-6">
          <div className="flex items-center justify-between border-b border-gray-200 pb-4 pt-24 dark:border-gray-800">
            <h1 className="text-xl font-bold tracking-tight sm:text-2xl">
              {prod.length} result{prod.length === 1 ?"" : "s"}
            </h1>
             <ProductSort /> 
          </div>
          ...
          <section aria-labelledby="products-heading" className="pb-24 pt-6">
            ...
            <div className={cn("grid grid-cols-1 gap-x-8 gap-y-10",prod.length> 0 ? 'lg:grid-cols-4' : 'lg:grid-cols-[1fr_3fr]')}>
